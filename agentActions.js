...
export async function handleAgenticActions(actions) {
    ...
    if (!initializeUI || !selectFile || !addMessageToUI) {
        ...
        if (addMessageToUI) {
            ...
            addMessageToUI(
                'assistant',
                'Internal Error: Could not execute actions due to missing UI functions.'
            );
        }
        return;
    }
    ...
    if (!fileStorage || typeof fileStorage.saveFile !== 'function') {
        console.error(
            '[handleAgenticActions] fileStorage is not available or doesn\'t have a saveFile method!'
        );
        addMessageToUI(
            'assistant',
            'Internal Error: File storage system is not available.'
        );
        return;
    }
    ...
    if (!Array.isArray(actions) || actions.length === 0) {
        console.warn(
            '[handleAgenticActions] Invalid or empty actions array received:',
            actions
        );
        ...
        try {
            ...
            case 'create_file':
                if (!action.path || typeof action.content !== 'string')
                    throw new Error(
                        'Missing path or content for file operation'
                    );
                result = await fileOps.createFile(
                    action.path,
                    action.content
                );
                if (result.success) {
                    lastAffectedPath = action.path;
                    actionsExecuted++;
                }
                break;
            case 'modify_file':
            case 'update_file':
                ...
                result = await fileOps.modifyFile(
                    action.path,
                    action.content
                );
                if (result.success) {
            ...
            case 'create_folder':
                if (!action.path)
                    throw new Error('Missing path for folder operation');
            ...
            case 'delete_file':
                ...
                break;
            case 'rename_file':
                if (!action.path || !action.new_path)
                    throw new Error(
                        'Missing source or destination path for rename operation'
                    );
                ...
                if (!result.success) {
                    result = await fileOps.renameFile(
                        action.path + '.folder',
                        action.new_path + '.folder'
                    );
                }
                if (result.success) {
                    lastAffectedPath = action.new_path;
                    actionsExecuted++;
                }
                break;
            case 'move_file':
                if (!action.path || !action.new_path)
                    throw new Error(
                        'Missing source or destination path for move operation'
                    );
                result = await fileOps.moveFile(
                    action.path,
                    action.new_path
                );
            ...
            case 'copy_file':
                if (!action.path || !action.new_path)
                    throw new Error(
                        'Missing source or destination path for copy operation'
                    );
            ...
            default:
                console.warn(
                    `[handleAgenticActions] Unknown agentic action type: ${action.type}`
                );
                // Optionally inform the user via addMessageToUI about the unknown action
                addMessageToUI(
                    'assistant',
                    `Info: Received an unknown action type "${action.type}".`
                );
            }
            if (!result.success && result.error) {
                addMessageToUI(
                    'assistant',
                    `⚠️ Error processing action (${action.type} ${action.path || ''}): ${result.error}`
                );
            }
        } catch (error) {
            console.error(
                `[handleAgenticActions] Error executing action: ${JSON.stringify(action)}`,
                error
            );
            addMessageToUI(
                'assistant',
                `⚠️ Error processing action (${action.type} ${action.path || ''}): ${error.message}`
            );
    ...
    if (actionsExecuted > 0) {
        addMessageToUI(
            'assistant',
            `✅ Executed ${actionsExecuted} file action(s).`
        );
        ...
        console.log(
            '[handleAgenticActions] No actions were successfully executed.'
        );
